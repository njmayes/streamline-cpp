...
namespace slc {
	...
	namespace Internal {
		...
		struct KeyCompare
		{
			...
			constexpr KeyCompare(const Key& k) noexcept : key(k) {}

			template<typename Element> requires HasKey<Element, Key>
		...
		public:
			...
			constexpr MapElement(const KeyType& key, const MappedType& value) noexcept : mPair(key, value) {}

			constexpr bool operator<(const MapElement& rhs) const noexcept { return mPair.first < rhs.mPair.first; }
			constexpr bool operator<(const CompareType& rhs) const noexcept { return mPair.first < rhs.key; }


			constexpr const auto& operator*() const noexcept { return mPair; }
			constexpr const auto* operator->() const noexcept { return &mPair; }

			constexpr void swap(MapElement& rhs) noexcept { std::swap(mPair, rhs.mPair); }
		...
		public:
			constexpr KeyCompareHash(const Key& key) noexcept : BaseType(key), mHash(Hasher()(key)) {
			}
		...
		public:
			constexpr StaticMapIterator(const Element* pos) noexcept : mPos(pos) {
			}

			constexpr bool operator==(const StaticMapIterator& rhs) const noexcept { return mPos == rhs.mPos; }
			constexpr bool operator!=(const StaticMapIterator& rhs) const noexcept { return mPos != rhs.mPos; }
			...
			constexpr StaticMapIterator operator+(std::size_t i) const noexcept { return mPos + i; }
			...
			constexpr std::size_t operator-(const StaticMapIterator& rhs) const noexcept { return mPos - rhs.mPos; }

			constexpr const auto& operator*() const noexcept { return **mPos; }
			constexpr const auto* operator->() const noexcept { return &**mPos; }
	...
	public:
		constexpr const MappedType& at(const KeyType& key) const noexcept { return find(key)->second; }

		constexpr std::size_t size() const noexcept { return N; }

		constexpr const_iterator begin() const noexcept { return mData; }
