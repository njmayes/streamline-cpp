...
namespace slc {
	...
	{
	public:
		template < ImVecType Vec, typename T >
			requires VecSized< Vec, T >
		static Vec ToImVec( const T& var )
		{
			return std::bit_cast< Vec >( var );
		}

		template < typename T, ImVecType Vec >
			requires VecSized< Vec, T >
		static T FromImVec( const Vec& var )
		{
		...
		static T WindowPos()
		{
			return FromImVec< T >( WindowPos() );
		}
		static ImVec2 WindowPos();

		template < typename T >
			requires VecSized< ImVec2, T >
		static void SetNextWindowSize( const T& size, ImGuiCond cond = ImGuiCond_FirstUseEver )
		{
			SetNextWindowSizeInternal( ToImVec< ImVec2 >( size ), cond );
		}
		template < typename T >
			requires VecSized< ImVec2, T >
		static void SetNextWindowPos( const T& size, const T& pivot = T( 0, 0 ), ImGuiCond cond = ImGuiCond_FirstUseEver )
		{
			SetNextWindowPosInternal( ToImVec< ImVec2 >( size ), ToImVec< ImVec2 >( pivot ), cond );
		}

		template < typename T >
			requires VecSized< ImVec4, T >
		static void SetButtonColour( const T& colour )
		{
			SetButtonColourInternal( ToImVec< ImVec4 >( colour ) );
		}
		static void SetButtonDefaults();

		template < typename T >
			requires VecSized< ImVec2, T >
		static T GetMainWindowCentre()
		{
			return FromImVec< T >( GetMainWindowCentreInternal() );
		}
		...
		template < typename T >
			requires VecSized< ImVec2, T >
		static T AvailableRegion()
		{
			return FromImVec< T >( AvailableRegion() );
		}
		static ImVec2 AvailableRegion();
		template < typename T >
			requires VecSized< ImVec2, T >
		static T AvailableRegionMax()
		{
			return FromImVec< T >( AvailableRegionMax() );
		}
		static ImVec2 AvailableRegionMax();
		template < typename T >
			requires VecSized< ImVec2, T >
		static T AvailableRegionMin()
		{
			return FromImVec< T >( AvailableRegionMin() );
		}
		...
		template < typename T >
			requires VecSized< ImVec2, T >
		static T FramePadding()
		{
			return FromImVec< T >( FramePadding() );
		}
