...
namespace slc {
	...
	public:
		...
		static void BeginChild( std::string_view strID )
		{
			BeginChild< ImVec2 >( strID );
		}
		...
		template < std::signed_integral T, typename Func >
			requires IsAction< T >
		...
		template < VecSized< ImVec2 > T >
		static void Image( uintptr_t image, const T& size, float rotation = 0.0f )
		{
			ImageInternal( ( ImTextureID )image, Utils::ToImVec< ImVec2 >( size ), rotation );
		}
		template < VecSized< ImVec2 > T >
		...
		template < typename T, typename Func >
			requires IsAction< Func, std::string_view, const T& >
		static void Combobox( std::string_view label, std::string_view preview, T value, std::span< const ComboEntry< T > > table, Func&& on_selection )
		{
			if ( !BeginCombo( label, preview ) )
				return;
			...
			on_selection( comboEntry->key, *( const T* )comboEntry->getVal() );
		}
