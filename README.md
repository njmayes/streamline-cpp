...
## Requirements
#### Core
* magic_enum support, i.e. `MAGIC_ENUM_SUPPORTED`/`magic_enum::is_magic_enum_supported` 

#### MSVC Only
* Requires `/Zc:preprocessor` flag.
...
* `STREAMLINE_BUILD_CHAT_DEMO` - Builds the chat room demo application.
* `STREAMLINE_BUILD_BENCHMARK` - Builds the benchmarking application for testing performence.
* `STREAMLINE_BUILD_TEST_APP` - Bulds the test app which contains compilation tests and some example code - mostly for development.
...
set(STREAMLINE_BUILD_CHAT_DEMO ON CACHE BOOL "Build chat room demo app" FORCE)
...
There are `SL/Core.h`, `SL/Gfx.h` and `SL/Net.h` headers which provide single headers for including all common headers in their respective modules. Individual headers can be found in `SL/Core/...` for example.
...
By executing the `sl::Application::Run` function, the application instance will be created and the application loop will be started on the current thread. By default this is a headless application with no GUI.
...
### Chat Room Demo Applicaiton
The chat room demo application provided demonstrates a simple demo of all modules. The application functions as a chat room server or client, depending on command line parameters. 

There is not currently a way to run the chat room without SSL, so users must generate their own certficates and keys which should be placed in the working directory directory for the application. You can create the cert/key pair by running:
...
The following is an example of the command line parameters you might use to run the demo as a chat room server:

```
./streamline-chat-demo server --ports=52612,62132 --cert=server.crt --key=server.key
```
...
The following is an example of the command line parameters you might use to run the demo as a chat room client:

```
./streamline-chat-demo client --host=127.0.0.1 --port=52612 --cert=server.crt
```
